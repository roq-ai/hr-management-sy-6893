...
import { getVacations, deleteVacationById } from 'apiSdk/vacations';
import { VacationInterface } from 'interfaces/vacation';

type ColumnType = ColumnDef<VacationInterface, unknown>;

interface VacationListPageProps {
...
export function VacationListPage(props: VacationListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<VacationInterface>>(
    () => `/vacations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: VacationInterface) => {
    if (hasAccess('vacation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/vacations/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/vacations/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('vacation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/vacations/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('vacation', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="vacation" />
        </Flex>

        {hasAccess('vacation', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/vacations/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}

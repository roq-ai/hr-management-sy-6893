...
export function PayrollListPage(props: PayrollListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PayrollInterface>>(
    () => `/payrolls?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'deduction', header: 'deduction', accessorKey: 'deduction' },
    {
      id: 'pay_date',
      header: 'Pay Date',
      accessorKey: 'pay_date',
      cell: ({ row: { original: record } }: any) =>
        record?.pay_date ? format(parseISO(record?.pay_date as unknown as string), 'dd-MM-yyyy') : '',
    },
